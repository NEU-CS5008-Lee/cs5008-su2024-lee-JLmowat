...
int main () {
  ...
  for (v = 0; v < GSIZE; v++) {
    ...
    T[v].lastStep = -1; // undefined
  }

  // DIJKSTRA: T[source].distance = 0
  // ********** INSERT YOUR CODE HERE **********
  T[0].distance = 0; // Starting point has a 0 path to itself
  ...
  while (setNotEmpty(Q,GSIZE)) {
    ...
    // ********** INSERT YOUR CODE HERE **********
    u = minDistance(T, GSIZE, Q); // get the vertex with the shortest distance
    ...
    // ********** INSERT YOUR CODE HERE **********
    setNeighbors(S, E, GSIZE, u); // save boolean for neighboring vertex


    // DIJKSTRA: Q = Q – u
    // ********** INSERT YOUR CODE HERE **********
    Q[u] = false; // remove the vertex from the while loop


    // DIJKSTRA: for each neighbor v of u (ignore vertices that we have already finished)
    for (v=0; v<GSIZE; v++) {
      if (S[v] && Q[v]) { // is a neighbor and hasn't been used as a source vertex
