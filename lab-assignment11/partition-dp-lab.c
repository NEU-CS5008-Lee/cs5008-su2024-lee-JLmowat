...
// 100 200 300 400 500 600 700 800 900
    ...
    for (i=2; i<=n; i++) {
        ...
        for (j=2; j<=k; j++) {
            // initialize all the cells to infinity
            m[i][j] = MAXINT;
            // x is the position of the dividers
            for (x=1; x<i; x++){
                cost = max(m[x][j-1],p[i]-p[x]);
                if(m[i][j]>cost) { // only the smallest maximum cost will be saved
                // store the cost and position of the dividers
                    m[i][j] = cost;
                    d[i][j] = x;
                }
