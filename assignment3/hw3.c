...
queue_t *newQueue()
{
  ...
  q_p = (queue_t *)malloc(sizeof(queue_t)); // Allocate memory to newly created queue
  if (q_p != NULL) {
    q_p->head_p = NULL;
    q_p->tail_p = NULL;
  }
...
void enqueue(queue_t *q_p, int d)
{
  ...
  if (q_p != NULL)
  {
    ...
    if (isEmpty(q_p))
    {
      ...
      n_p = newNode(d); // only assigns data value left and right pointers are NULL
    ...
    else
    {
      ...
      // Allocate memory to new node pointer
      ...
      // Tail pointer holds left_p = NULL, Data, right_p
      // When updating Tail only the left_p is udpates
  ...
  if (q_p != NULL)
  {
    ...
    if (n_p != NULL)
    {
      ...
      if (q_p->head_p == q_p->tail_p)
      {
        ...
        q_p->head_p = NULL;
        q_p->tail_p = NULL;
        //free(q_p); // freeQueue does not work because it would call dequeue
      }
      else
      {
        ...
        // ***** INSERT YOUR CODE HERE *****
        // Reassign the Head pointer to the node to the left
        q_p->head_p = q_p->head_p->left_p;
        // Head pointers right_p always points to NULL
